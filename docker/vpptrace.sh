...
function get_vpp_input_node {
    ...
    if [[ "$1" == "ipsec" ]] || [[ "$1" == "encrypt"* ]]; then
        ...
        echo "dpdk-esp4-encrypt-tun"
        echo "dpdk-esp6-encrypt-tun"
        echo "dpdk-esp4-encrypt"
        echo "dpdk-esp6-encrypt"
    fi
    if [[ "$1" == "ipsec" ]] || [[ "$1" == "decrypt"* ]]; then
        echo "dpdk-esp6-decrypt"
        echo "dpdk-esp6-decrypt-post"
        echo "dpdk-esp4-decrypt"
        echo "dpdk-esp4-decrypt-post"
    fi

    echo "iptfs-output"
    echo "iptfs-encap"
    echo "iptfs-encap-enq"
    echo "iptfs-encap4-tun"
    echo "iptfs-encap6-tun"
    echo "iptfs-decap"
    echo "iptfs-decap-reorder"
    echo "iptfs-zpool-poller"
    echo "iptfs-zpool-process"
    echo "iptfs-zpool-process"
...
function trace {
    ...
    while getopts i:a:rf:p:h option
    do
        case "${option}"
            in
            i) INTERFACES+=(${OPTARG});;
    ...
    if [ "${#INTERFACES[@]}" -eq 0 ]; then
        INTERFACES=(dpdk af-packet ipsec tap)
    fi

    # connect to VPP-CLI
    VPPCONFIG="/etc/vpp/startup.conf"
    if [[ -e ${VPPCONFIG} ]]; then
        if [[ -z "$PORT" ]]; then
